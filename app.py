...
# 1. 환경 설정
st.set_page_config(page_title="주식 AI v53.0 (3Y Learning)", layout="wide")
...
# --- [3. 지능 누적 로직] ---
def get_accumulated_fluid_coef():
    if os.path.exists(CONFIG_PATH):
        try:
            df = pd.read_csv(CONFIG_PATH)
            if not df.empty:
                return max(0.15, df['best_m_coef'].mean())
        except: pass
    return 0.15

# --- [4. 메인 분석 엔진] ---
st.title("🏛️ 주식 AI v53.0 (3년 학습 및 1개월 집중 대시보드)")
...
with st.sidebar:
    ...
    st.metric("데이터 유지 지능 (축적치)", f"{get_accumulated_fluid_coef():.4f}")
...
if st.button("3년 데이터 학습 및 한 달 집중 분석 시작", use_container_width=True):
    success_flag = False
    try:
        with st.status("3년치 장기 데이터를 분석하여 지능을 점진적으로 강화 중...", expanded=True) as status:
            # 1. 3년(1095일) 주가 데이터 수집
            start_date = KST_NOW - timedelta(days=1095)
            df_raw = fdr.DataReader(s_code, start_date).rename(columns={'Close':'종가','Volume':'거래량'})
            
            # 2. 뉴스 인과관계 전수 조사 (최근 60거래일 집중 학습)
            ...
            accumulated_coef = get_accumulated_fluid_coef()
            ...
            if not knowledge_df.empty and all(col in knowledge_df.columns for col in features):
                ...
                model = Ridge(alpha=0.2).fit(X_total, y_total, sample_weight=np.concatenate([np.ones(len(df_final)), np.full(len(knowledge_df), get_accumulated_fluid_coef())]))
            else:
            ...
            st.session_state.result = {
                'df': df_final.tail(30), # 가시적 그래프는 최근 한 달(30일)만 표시
                'f_prices': f_prices,
            ...
            df_final[features + ['target', 'stock_code']].tail(50).to_csv(DB_PATH, mode='a', header=not os.path.exists(DB_PATH), index=False)
            success_flag = True
            status.update(label="3년 학습 및 1개월 집중 분석 완료!", state="complete")
...
if 'result' in st.session_state:
    ...
    st.subheader(f"📊 최근 1개월 집중 리포트 (3년 장기 학습 모델)")
    fig = go.Figure()
