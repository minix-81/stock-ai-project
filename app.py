...
# 1. 페이지 설정
st.set_page_config(page_title="주식 AI v6.3 (정밀 분석)", layout="wide")
...
with st.sidebar:
    st.title("🚀 데이터 센터")
    menu = st.radio("메뉴", ["실전 분석", "관리자"], key="main_nav_v63")
    ...
    if st.button("온라인 지능 통합 분석 시작", width='stretch'):
        try:
            ...
            if not g_df.empty:
                ...
                st.info(f"💡 공유 지능 {len(g_df)}행을 통합 학습했습니다.")
            else:
            ...
            f_prices, f_dates = [], []
            temp_p, last_feat = last_p, df_curr[features].iloc[-1:].copy()
            ...
            st.success("분석 완료! 깨달은 지식이 온라인 DB에 저장되었습니다.")
...
# --- [페이지 2: 관리자 (통계 기능)] ---
    ...
    if st.text_input("비번", type="password") == "0801":
        ...
        if not g_data.empty:
            ...
            st.info("아직 온라인 DB에 저장된 데이터가 없습니다. 분석을 먼저 실행하세요.")
