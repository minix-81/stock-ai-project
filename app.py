...
# 1. 페이지 설정
st.set_page_config(page_title="주식 AI v8.5 (최종 완성)", layout="wide")
KST_NOW = datetime.now() + timedelta(hours=9)

# 2. Supabase 연결 (Secrets 연동 강화)
try:
...
with st.sidebar:
    ...
    if conn:
        try:
            ...
            st.sidebar.warning("⚠️ 연결은 되었으나 응답이 없습니다.")
    else:
        st.sidebar.error("❌ Secrets 설정을 확인하세요.")
    
    menu = st.radio("메뉴 선택", ["실전 분석", "관리자"], key="nav_v85")
...
if menu == "실전 분석":
    ...
    if st.button("분석 및 지능 공유 시작", width='stretch'):
        try:
            # [단계 1] 2년치 데이터 수집 및 지표 생성
            ...
            if save_to_db(stock_code, df_curr):
                ...
                st.warning("⚠️ 분석은 완료됐으나 DB 저장은 실패했습니다.")
...
# --- [페이지 2: 관리자] ---
    ...
    if st.text_input("관리자 비번", type="password") == "0801":
        ...
        if not g_data.empty:
            ...
            # image_4f932b.png 대응
            st.info("아직 저장된 데이터가 없습니다. 실전 분석을 먼저 실행하세요.")
