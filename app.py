...
# 1. 환경 설정
st.set_page_config(page_title="주식 AI v49.0 (Optimized Sentiment)", layout="wide")
KST_NOW = datetime.now() + timedelta(hours=9)
DB_PATH = "stock_knowledge_v49.csv"
NEWS_DB_PATH = "news_rss_cache_v49.csv"
CONFIG_PATH = "global_config_v49.csv"
...
# --- [4. 메인 분석 엔진] ---
st.title("🏛️ 주식 AI v49.0 (투자 심리 기반 인과관계 분석)")
...
if st.button("투자 심리 통합 분석 및 미래 예측 시작", use_container_width=True):
    success_flag = False
    try:
        with st.status("투자 심리와 기술적 지표의 인과관계를 통합 학습 중...", expanded=True) as status:
            ...
            # [내부 로직] 뉴스 심리 반영비 660배 조정 및 3일 누적
            ...
            success_flag = True
            status.update(label="최적화 분석 및 미래 예측 완료!", state="complete")
