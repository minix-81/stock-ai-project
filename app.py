...
# 1. 환경 설정
st.set_page_config(page_title="주식 AI v28.0 (Sentiment Master)", layout="wide")
...
# --- [5. 메인 분석 로직] ---
st.title("🏛️ 주식 AI v28.0 (초정밀 누적 감성 엔진)")
c1, c2 = st.columns(2)
...
if st.button("백테스팅 및 7일 미래 예측 시작", use_container_width=True):
    ...
    try:
        with st.status("AI가 3일간의 뉴스를 교차 분석하며 학습 중...", expanded=True) as status:
            ...
            df['뉴스감성'] = temp_scores.rolling(window=3, min_periods=1).mean() * 150 # 영향력 증폭
            ...
            st.session_state.result = {
                ...
                'news_score': df_final['뉴스감성'].iloc[-1] / 150
            }
            ...
            success_flag = True
            status.update(label="학습 및 예측 완료!", state="complete")
