...
# 1. 페이지 설정
st.set_page_config(page_title="주식 AI v7.1 (완전판)", layout="wide")
...
with st.sidebar:
    st.title("🚀 AI 진단 센터")
    menu = st.radio("메뉴 선택", ["실전 분석", "관리자"], key="main_nav_v71")
    
    st.divider()
    if st.button("🔍 DB 연결 상태 점검"):
        if conn:
            try:
                # knowledge 테이블 존재 여부 확인
    ...
    if st.button("AI 분석 시작", width='stretch'):
        try:
            ...
            if save_to_db(stock_code, df_curr):
                st.success("✅ 분석 완료! 지식이 온라인 DB에 영구 저장되었습니다.")
...
# --- [페이지 2: 관리자] ---
    ...
    if st.text_input("관리자 비번", type="password") == "0801":
        ...
        if not g_data.empty:
            ...
            st.info("현재 저장된 데이터가 없습니다. 실전 분석을 실행해 보세요.")
