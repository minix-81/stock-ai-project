...
# 1. 페이지 설정
st.set_page_config(page_title="주식 AI v8.2 (완성판)", layout="wide")
...
with st.sidebar:
    ...
    menu = st.radio("메뉴 선택", ["실전 분석", "관리자"], key="nav_v82_final")
    ...
    if st.button("분석 및 지능 공유 시작", width='stretch'):
        try:
            ...
            if save_to_db(stock_code, df_curr):
                ...
                st.warning("⚠️ 분석은 끝났으나 DB 저장은 실패했습니다.")
