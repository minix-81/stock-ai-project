...
# 1. 페이지 설정
st.set_page_config(page_title="주식 AI v6.5 (완성판)", layout="wide")
...
with st.sidebar:
    st.title("🚀 AI 데이터 센터")
    menu = st.radio("메뉴 선택", ["실전 분석", "관리자"], key="nav_final_v65")
    st.info(f"접속 시간(KST): {KST_NOW.strftime('%Y-%m-%d %H:%M:%S')}")

# --- [데이터 관리 함수] ---
def load_db():
    if conn:
        try:
            # image_4e3aa1.png의 'knowledge' 테이블 로드
...
if menu == "실전 분석":
    ...
    if st.button("AI 분석 및 온라인 지능 통합 시작", width='stretch'):
        try:
            ...
            st.success("분석 완료! 깨달은 지식이 온라인 DB에 영구 기록되었습니다.")
