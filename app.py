...
# 1. 환경 설정
st.set_page_config(page_title="주식 AI v52.4 (Analysis Report)", layout="wide")
...
def get_learning_volume():
    if os.path.exists(DB_PATH):
        try:
            return len(pd.read_csv(DB_PATH))
        except: pass
    return 0

# --- [2. 메인 분석 엔진] ---
st.title("🏛️ 주식 AI v52.4 (실전형 통합 지능 분석)")
...
if st.button("분석 시작", use_container_width=True):
    success_flag = False
    try:
        with st.status("AI 학습 및 인과관계 분석 중...", expanded=True) as status: # [수정] 간결한 문구
